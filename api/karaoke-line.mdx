---
title: "KaraokeLine: syllable-level lyrics line types"
...
<ParamField path="end" type="Int" required>
  End time in milliseconds. Must be ≥ `start`.
</ParamField>
...
A unified copy helper that works across both `KaraokeLine` subtypes without requiring a `when` expression. Delegates to the data class `copy()` of the underlying concrete type, preserving the runtime type.
